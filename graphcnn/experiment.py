...
class GraphCNNExperiment(object):
    ...
    def __init__(self, data_set_name, model_name, net_constructor):
        ...
        self.debug = False
        self.starter_learning_rate = 0.5##
    ...
    def run_kfold_experiments(self, no_folds=10,random_state=125):
        ...
        for i in range(no_folds):
            ...
            cur_max, sensitivity_max, specificity_max, f1_max,auc_max,max_it= self.run()#, attr#run2
            self.print_ext('Fold %d max accuracy: %g  sensitivity: %g specificity: %g f1_score: %g auc: %g at %d' % (
                i, cur_max, sensitivity_max, specificity_max, f1_max,auc_max,max_it))
            acc.append(cur_max)
            sensitivity.append(sensitivity_max)
            specificity.append(specificity_max)
            f1_score.append(f1_max)
            auc.append(auc_max)
            ...
            with open('./results/%s.txt' % self.data_set_name, 'a+') as file:
                file.write('%s\t%d-fold\t%.4f sensitivity\t%.4f specificity\t%.4f f1_score\t%.4f auc\t%.4f \n' % (
                    str(datetime.now()), i, cur_max, sensitivity_max, specificity_max,f1_max,auc_max))
        ...
        mean_auc=np.mean(auc)*100
        std_auc=np.std(auc)*100
        ...
        self.print_ext('f1 is: %.2f (+- %.2f)' % (mean_f1_score, std_f1_score))
        self.print_ext('auc is: %.2f (+- %.2f)' % (mean_auc, std_auc))
        verify_dir_exists('./results/')
        with open('./results/%s.txt' % self.data_set_name, 'a+') as file:
            file.write('%s\t%s\t%d-fold\t%d seconds\t%.2f (+- %.2f)\n' % (
                str(datetime.now()), desc, no_folds, time.time() - start_time, mean_acc, std_acc))
            return mean_acc, std_acc, mean_sensitivity, std_sensitivity, mean_specificity, std_specificity,mean_f1_score,std_f1_score,mean_auc,std_auc#, attr_arr
    ...
    def create_loss_function(self):
        with tf.variable_scope('loss') as scope:
            ...
            mask=tf.equal(tp+fp,tf.constant(0,dtype=tf.float32))
            precision=tf.cond(mask,lambda:tf.constant(0,dtype=tf.float32),lambda:tp/(tp+fp))
            mask_sen=tf.equal(sensitivity,tf.constant(0,dtype=tf.float32))#+precision
            mask_pre=tf.equal(precision,tf.constant(0,dtype=tf.float32))
            f1=tf.cond(mask_sen,lambda:
                tf.cond(mask_pre,lambda:tf.constant(0,dtype=tf.float32),lambda:2 * (precision * sensitivity) / (precision + sensitivity)),
                lambda:2 * (precision * sensitivity) / (precision + sensitivity))
            auc,update_op=tf.metrics.auc(self.net.labels,tf.argmax(self.net.current_V, 1))
            ...
            self.max_f1_train=tf.Variable(tf.zeros([]), name="max_f1_train")
            self.max_f1_test=tf.Variable(tf.zeros([]), name="max_f1_test")
            self.max_auc_train=tf.Variable(tf.zeros([]), name="max_auc_train")
            self.max_auc_test=tf.Variable(tf.zeros([]), name="max_auc_test")
            ...
            max_f1=tf.cond(self.net.is_training, lambda: tf.cond(tf.greater(accuracy, max_acc),
                                                                            lambda: tf.assign(
                                                                                self.max_f1_train,
                                                                                f1),
                                                                            lambda: self.max_f1_train),
                                      lambda: tf.cond(tf.greater(accuracy, max_acc),
                                                      lambda: tf.assign(self.max_f1_test, f1),
                                                      lambda: self.max_f1_test))
            max_auc=tf.cond(self.net.is_training, lambda: tf.cond(tf.greater(accuracy, max_acc),
                                                                            lambda: tf.assign(
                                                                                self.max_auc_train,
                                                                                auc),
                                                                            lambda: self.max_auc_train),
                                      lambda: tf.cond(tf.greater(accuracy, max_acc),
                                                      lambda: tf.assign(self.max_auc_test, auc),
                                                      lambda: self.max_auc_test))
            ...
            tf.summary.scalar('max_f1', max_f1)
            tf.summary.scalar('max_auc', max_auc)
            ...
            self.reports['max_f1'] = max_f1
            self.reports['update_op'] = update_op
            self.reports['max_auc'] = max_auc
            ...
            with tf.Session() as sess:
                ...
                try:
                    ...
                    while i < self.num_iterations:
                        ...
                        if i % self.iterations_per_test == 0:  # and i >= 700:i>=500 and  
                ...
                return sess.run(
                    [self.max_acc_test, self.max_sensitivity_test, self.max_specificity_test, self.max_f1_test,self.max_auc_test,self.net.global_step,
                     ], feed_dict={self.net.is_training: 0})#self.net.attr
        else:
